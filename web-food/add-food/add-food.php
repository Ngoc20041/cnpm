<?php
// Bắt đầu hoặc khôi phục một phiên làm việc
session_start();
// Kiểm tra xem phiên làm việc có tồn tại không
include('../db/connect.php');

?>

<!DOCTYPE html>
<html lang="en">

<head>
    <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200" />
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width,  initial-scale=1.0">
    <title>Của hàng của bạn</title>
    <link rel="stylesheet" href="add-food.css">
    <link href="https://fonts.googleapis.com/css2?family=Poppins:ital,wght@0,500;1,400&display=swap" rel="stylesheet">
    <script src="home.js"></script>
</head>

<body>
    <header>
        <div id="img">
            <a href="../home/home.php" title="logo">
                <img width="40" src="data:image/png;base64," alt="VietNamNet">
            </a>
        </div>

        <div id="khoi2">
            <nav class="navi">
                <a class="a" href="../home/home.php">Trang chủ</a>
                <a class="a" href="../page-food/page-food.php">Đồ ăn</a>
                <a class="a" href="../cart/page-cart.php">Giỏ hàng</a>
                <a class="a" id="gach-chan-your-shop" href="../add-food/add-food.php">Cửa hàng của bạn</a>
            </nav>
        </div>

        <div class="user-info">
            <p>Hello <span id="username">
                    <?php
                    if (isset($_SESSION['username']) && !empty($_SESSION['username'])) {
                        echo $_SESSION['username'];
                    } else {
                        echo ' Guest!';
                    }
                    ?>
            </p>
            <form id="form-logout" method="post" action="../home-not-login/home-not-login.php">
                <input type="submit" value="Logout" class="" name="logout">
            </form>
        </div>

    </header>



    <!-- <div id="banner">
        <img class="img-banner" width="100%" height="100%" src="../images/banner.png" alt="banner">
        <h2>NĂM MỚI VUI VẺ !!!</h2> 
    </div>   -->
    <div class="khung-danh-muc">
        <div class="h2-danh-muc">
            <h2>Danh mục</h2>
        </div>

        <div class="danh-muc">
            <p>
            <ul class="menu-danh-muc">
                <br>
                <li> <input class="submit-menu" type="submit" name="them-san-pham" value="Thêm sản phẩm"></li>
            </ul>
            </p>
        </div>
        <div class="danh-muc">
            <p>
            <ul class="menu-danh-muc">
                <br>
                <li> <input class="submit-menu" type="submit" name="chinh-sua" value="Chỉnh sửa"></li>
            </ul>
            </p>
        </div>
        <div class="danh-muc">
            <p>
            <ul class="menu-danh-muc">
                <br>
                <li> <input class="submit-menu" type="submit" name="xoa" value="Xóa"></li>
            </ul>
            </p>
        </div>
        <div class="danh-muc">
            <p>
            <ul class="menu-danh-muc">
                <br>
                <li>
                    <form id="search-form" method="POST">
                        <input type="text" name="dulieu" placeholder="Tìm kiếm..." required>
                        <input type="submit" name="submit" value="Tìm kiếm">
                    </form>

                    <?php

                    function hienthitimkiem()
                    {
                        include('../db/connect.php');
                        if (isset($_POST['submit'])) {
                            $noidung = $_POST['dulieu'];
                        } else {
                            echo $noidung = "Không có dữ liệu   ";
                        }
                        // Truy vấn cơ sở dữ liệu để lấy thông tin
                        $sql = "SELECT * FROM tblfood WHERE name LIKE '%$noidung%' ";
                        $result = mysqli_query($mysqli, $sql);

                        // Kiểm tra và hiển thị thông tin
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {

                                echo "<p  style='margin-left:20px;font-size: 14px;'> " . $row['img'] . "</p>";
                                echo "<p style='margin-left:20px;font-size: 14px;'> Tên món ăn: " . $row['name'] . "</p>";
                                echo "<p style='margin-left:20px;font-size: 14px;'> Mô tả: " . $row['discribe'] . "</p>";
                                echo "<p style='margin-left:20px;font-size: 14px;'> Giá Tiền:  " . $row['price'] . "</p>";
                                echo "<hr>";
                            }
                        } else {
                            //echo "Không có dữ liệu";
                        }

                        $mysqli->close();
                    }
                    hienthitimkiem();
                    ?>
                </li>
            </ul>
            </p>
        </div>


    </div>
    <div class="form-add-san-pham">
        <h2>Thêm sản phẩm</h2>
        <form action="../add-food/reg-add-food.php" method="POST">

            <label for="name">Tên sản phẩm:</label><br>
            <input type="text" id="name" name="name" required>
            <br>

            <label for="img">Link ảnh:</label><br>
            <input type="file" id="img" name="img" required>
            <br>


            <label for="mo-ta">Mô tả sản phẩm:</label><br>
            <input type="text" id="mo-ta" name="mo-ta" required>
            <br>


            <label for="gia">Giá:</label><br>
            <input type="text" id="gia" name="gia" required>
            <br>

            <input type="submit" name="them" value="Thêm">

        </form>
    </div>
    <div class="cac-san-pham-vua-them">
        <h2>Các sản phẩm vừa thêm</h2>
        <?php
        function hienthi()
        {
            include('../db/connect.php');

            // Truy vấn cơ sở dữ liệu để lấy thông tin
            $sql = "SELECT * FROM tblfood";
            $result = $mysqli->query($sql);

            // Kiểm tra và hiển thị thông tin
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {

                    echo "<p  style='margin-left:20px;font-size: 14px;'> " . $row['img'] . "</p>";
                    echo "<p style='margin-left:20px;font-size: 14px;'> Tên món ăn: " . $row['name'] . "</p>";
                    echo "<p style='margin-left:20px;font-size: 14px;'> Mô tả: " . $row['discribe'] . "</p>";
                    echo "<p style='margin-left:20px;font-size: 14px;'> Giá Tiền:  " . $row['price'] . "</p>";
                    echo "<hr>";
                }
            } else {
                echo "Không có dữ liệu";
            }

            $mysqli->close();
        }

        ?>
        <div class="list-san-pham">
            <ul>
                <li>
                    <?php

                    hienthi();
                    ?>

                </li>
            </ul>


        </div>

    </div>

    <footer>

        <h4>Xin chào tôi là admin, đây là web demo shop bán đồ ăn</h4>
        <div class="infor">
            <p>
            <h3>Trang cá nhân của các thành viên:</h3><br>
            <div class="li">
                <ul class="list">
                    <li> <a href="#">Phạm Thế Nhật </a></li>
                    <li> <a href="#">Bùi Bảo Ngọc </a></li>
                    <li> <a href="#">Nguyễn Thị Xuân Anh </a></li>
                    <li> <a href="#">Nguyễn văn Huynh </a></li>
                </ul>
            </div>

            </p>
        </div>
        <!-- Your footer content here -->
    </footer>

</body>

</html>